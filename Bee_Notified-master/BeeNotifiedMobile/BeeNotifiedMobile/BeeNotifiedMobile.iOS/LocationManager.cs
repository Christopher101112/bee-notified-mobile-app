...
namespace Location.iOS
{
    public class LocationManager
    {
        ...
        public async void PushLocToDB(bool sync)
        {
            if (App.ClientUser != null)
            {
                Console.WriteLine("Within PushLocToDB, Updating Location");
                updateLocEntity = App.ClientUser;
                updateLocEntity.Lat = LocMgr.Location.Coordinate.Latitude;
                updateLocEntity.Lon = LocMgr.Location.Coordinate.Longitude;
                await manager.SaveUserID(updateLocEntity);
            }

            else
            {
                Console.WriteLine("Within PushLocToDB, Not updating location");
            }
